...
export class CategoryService {
  ...
  async createCategory(createCategoryDto: CreateCategoryDto) {
    try {
      ...
      const data = await this.model.findOne({where:{name:createCategoryDto.name}})
      if(data){
  ...
  async updateCategoryById(id:number, updateCategoryDto:UpdateCategoryDto){
    try {
      ...
      const updatedCategory = await this.model.update(updateCategoryDto, {where:{id},returning: true})
      return {
        statusCode: 200,
        message: 'success',
